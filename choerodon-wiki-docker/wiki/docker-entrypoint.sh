...
function configure() {
  ...
  file_env 'DB_HOST' 'db'
  file_env 'DB_PORT' 'db_port'
  file_env 'DB_DATABASE' 'xwiki'
  file_env 'INDEX_HOST' 'localhost'
  file_env 'INDEX_PORT' '8983'
  file_env 'OIDC_ENDPOINT_AUTHORIZATION' 'http://localhost:8080/oauth/oauth/authorize'
  file_env 'OIDC_ENDPOINT_TOKEN' 'http://localhost:8080/oauth/oauth/token'
  file_env 'OIDC_ENDPOINT_USERINFO' 'http://localhost:8080/iam/v1/users/self'
  file_env 'OIDC_ENDPOINT_LOGOUT' 'http://localhost:8080/oauth/logout'
  file_env 'OIDC_CLIENTID' 'wiki'
  file_env 'OIDC_SECRET' 'secret'
  file_env 'OIDC_CUSTOMSCOPE' ''
  ...
  safesed "replaceport" $DB_PORT /usr/local/tomcat/webapps/ROOT/WEB-INF/hibernate.cfg.xml
  ...
  echo ' Setting auth config class'
  xwiki_set_cfg 'xwiki.authentication.authclass' "org.xwiki.contrib.oidc.auth.OIDCAuthServiceImpl"

  echo ' Setting xwiki.store.cache.capacity'
  xwiki_set_cfg 'xwiki.store.cache.capacity' '3000'

  echo ' Setting xwiki.authentication.group.allgroupimplicit'
  xwiki_set_cfg 'xwiki.authentication.group.allgroupimplicit' '1'

  echo ' Setting xwiki.backlinks'
  xwiki_set_cfg 'xwiki.backlinks' '0'

  echo ' Setting xwiki.monitor'
  xwiki_set_cfg 'xwiki.monitor' '0'

  echo ' Setting xwiki.rendering.defaultCacheDuration'
  xwiki_set_cfg 'xwiki.rendering.defaultCacheDuration' '3600'
  ...
  echo '  Configure choerodon OAuth'
  xwiki_set_properties 'oidc.endpoint.authorization' "$OIDC_ENDPOINT_AUTHORIZATION"
  xwiki_set_properties 'oidc.endpoint.token' "$OIDC_ENDPOINT_TOKEN"
  xwiki_set_properties 'oidc.endpoint.userinfo' "$OIDC_ENDPOINT_USERINFO"
  xwiki_set_properties 'oidc.endpoint.logout' "$OIDC_ENDPOINT_LOGOUT"
  xwiki_set_properties 'oidc.clientid' "$OIDC_CLIENTID"
  xwiki_set_properties 'oidc.clientsecret' "$OIDC_SECRET"
  xwiki_set_properties 'oidc.customscope' "$OIDC_CUSTOMSCOPE"
